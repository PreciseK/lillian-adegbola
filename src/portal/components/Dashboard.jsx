...
const Dashboard = ({ user }) => {
  ...
  const fetchUserDashboardData = async () => {
    try {
      ...
      // 2. Fetch Recent Orders (as proxy for activity)
      ...
      if (recentOrders) {
        combinedActivity.push(...recentOrders.map(o => ({
          id: `order-${o.id}`,
          action: 'Purchased item',
          item: `Order #${o.id.slice(0, 8)}`,
      ...
      if (recentGoals) {
        combinedActivity.push(...recentGoals.map(g => ({
          id: `goal-${g.id}`,
          action: 'Updated goal',
          item: g.title,
      ...
      const formattedActivity = combinedActivity.slice(0, 5).map(act => ({
        ...act,
      ...
      setDashboardData({
        stats: {
          courses: ordersCount || 0,
          hours: 0, // Placeholder until progress tracking is detailed
          downloads: 0, // Placeholder
          points: (ordersCount || 0) * 100 + (goalsCount || 0) * 50 + (postsCount || 0) * 10
        },
        learning: [], // We'd fetch enrolled courses here if we had the table
  ...
  const stats = [
    ...
    {
      title: 'Hours',
      value: dashboardData.stats.hours.toString(),
      change: 'Learned',
      icon: FiClock,
      color: 'emerald',
      trend: 'neutral'
    ...
    {
      title: 'Points',
      value: dashboardData.stats.points.toLocaleString(),
      change: 'Earned',
      icon: FiStar,
      color: 'gold',
      trend: 'up'
  ...
  const quickActions = [
    {
      ...
      icon: FiBook,
      color: 'blue',
      link: '/portal/courses', // Fixed link
    ...
    },
    {
      ...
      icon: FiBarChart3,
      color: 'purple',
      link: '/portal/analytics', // Fixed link
    ...
    },
    {
      ...
      color: 'orange',
      link: '/portal/community', // Fixed link
  ...
  // Current learning derived from state
  const currentLearning = dashboardData.learning;
      ...
      <div className="px-3 py-3 space-y-3 overflow-hidden">
              ...
              <div className="grid grid-cols-1 gap-2 mt-3">
                ...
                <div className="p-2 bg-luxury-pearl rounded">
                  ...
                  <UsageIndicator
                    membershipTier={membershipTier}
                    feature="downloads"
                    currentUsage={dashboardData.stats.downloads}
                    className="text-xs"
                  />
                </div>
              </div>
        ...
        >
          ...
          {currentLearning.length === 0 ? (
            <div className="text-center py-4 bg-luxury-pearl rounded">
              <p className="text-xs text-gray-500 font-montserrat mb-2">You haven't enrolled in any courses yet.</p>
              <Link to="/portal/courses" className="text-gold-600 text-xs font-semibold hover:underline">Start Learning</Link>
            </div>
          ) : (
                ...
                <div key={item.id} className="p-2 bg-luxury-pearl rounded">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center space-x-1 text-xs">
                      ...
                      <span className={`px-1 py-0.5 rounded text-xs font-medium ${item.type === 'course' ? 'bg-blue-100 text-blue-700' : 'bg-emerald-100 text-emerald-700'
                        }`}>
                        {item.type}
                      </span>
                    </div>
        ...
        >
          ...
          <div className="space-y-2">
            {recentActivity.length === 0 ? (
              <p className="text-xs text-gray-500 font-montserrat italic p-2">No recent activity.</p>
            ) : (
